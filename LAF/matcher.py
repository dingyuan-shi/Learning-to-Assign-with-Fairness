...
if sys.platform == 'darwin':
    from model.utils import finish_prob, get_layer_id, discrete_time, cal_online_seconds
    from model.entity import Pair, Driver, Order
    from model.global_var import lng_step, lat_step, DIRS, DIR_NUM, NUM_TOPK, TIME_INTERVAL, IS_CLEAR
    from model.KM import find_max_match
else:
    from utils import finish_prob, get_layer_id, discrete_time, cal_online_seconds
...
class Matcher:
    ...
    def dispatch(self, dispatch_observ: List[Dict[str, Any]], index2hash,
        ...
        olt = {driver.driver_id: cal_online_seconds(drivers_log_on_off[index2hash[driver.driver_id]][0], cur_sec,
                                                    per_hour=True)
               for driver in drivers.values()}
        inc = {driver.driver_id: drivers_income_per_hour[index2hash[driver.driver_id]][cur_time.tm_hour]
               for driver in drivers.values()}
